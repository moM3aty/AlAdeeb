...
namespace AlAdeeb.Data
{
    public class AppDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<QuestionBankSection>().HasOne(s => s.Course).WithMany(c => c.QuestionBankSections).HasForeignKey(s => s.CourseId).OnDelete(DeleteBehavior.Cascade);
            modelBuilder.Entity<BankQuestion>().HasOne(q => q.Section).WithMany(s => s.Questions).HasForeignKey(q => q.QuestionBankSectionId).OnDelete(DeleteBehavior.Cascade);
        }
    }
}